...
float CalculateIrisDiameter(const NormalizedLandmarkList& landmarks,
  ...
  // EDITED THIS ####
  std::cout<< "USE ME:: distance vertical " << dist_vert << std::endl;
  std::cout<< "USE ME:: distance horizontal " << dist_hori << std::endl;

  float average = (dist_hori + dist_vert) / 2.0f;

  //open file for writing
  std::ofstream fw("iris_out.txt", std::ofstream::out | std::ofstream::app);
  //exception handling
  try {
