...
// ABSL_DECLARE_FLAG(double, focal_length);
...
absl::StatusOr<double> ComputeFocalLengthInPixels(int image_width,
  ...
  // std::cout << focal_length_35mm ;
  // std::cout << focal_length_mm;
  // std::cout << fl_int ;
  // float temp = fl_35_int ; 
  // int focal_length_35mm = int fl_int;
  // int fl_35_int ;
  ...
  if (focal_length_mm == 0) {
    return focal_length_mm = int(1);
    // return absl::InternalError(
    //     "Focal length in mm should be non-zero to compute focal length in "
    //     "pixels.");
  }
  if (focal_length_35mm == 0) {
    return focal_length_35mm = int(2);
    // std::cout << 'HELLLOOOOOOO I AM HEREEEEEEEE' << std::endl;
